...
public class Queries {
	...
	// read classrooms
	public static final String selectClassroomStudents = "select  u.user_id , u.name , u.email , u.registered_at , cu.joined_at, count(distinct t.test_id) as totalTestsCount, count(distinct a.test_id) as attemptedTestsCount from Users u join Classroom_Users cu on  u.user_id = cu.user_id join Tests t on t.classroom_id=cu.classroom_id and t.status = 'published'  left join Attempts a on t.test_id=a.test_id and a.status !='started'  and a.user_id = cu.user_id  where cu.classroom_id = ? and cu.role = 'student' group by u.name,u.user_id;";
