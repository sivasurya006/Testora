...
public class Queries {
	...
	// read classrooms
	public static final String selectClassroomStudents = "select  u.user_id , u.name , u.email , u.registered_at , cu.joined_at, count(distinct t.test_id) as totalTestsCount, count(distinct a.test_id) as attemptedTestsCount from Users u join Classroom_Users cu on  u.user_id = cu.user_id left join Tests t on t.classroom_id=cu.classroom_id and t.status = 'published'  left join Attempts a on t.test_id=a.test_id and a.status !='started'  and a.user_id = cu.user_id  where cu.classroom_id = ? and cu.role = 'student' group by u.name,u.user_id";
...
//	public static final String selectClassroom=" select c.name, u.name , c.created_at from Classrooms c join Users u on created_by=user_id ";
	...
	public static final String selectStudentTests="select  t.test_id as testId ,t.title as testTitle, t.is_timed, t.duration_minutes, t.creator_id, t.maximum_attempts,  t.correction_type,t.classroom_id, u.name as creatorName, COUNT(a.attempt_id) as attemptCount, t.created_at as createdAt,a.status as status, a.user_id as userId from Tests t join Users u    on u.user_id = t.creator_id left join Attempts a    on a.test_id = t.test_id  and  a.user_id=? where t.classroom_id=? and t.status='published' group by  t.test_id,  t.title,    t.is_timed,   t.duration_minutes,   t.maximum_attempts,  t.correction_type,  u.name,a.status;";
	public static final String getDashBoardAnaliticsData="select  t.title , count(a.test_id) as totalAttemptsCount , count(distinct a.user_id) as totalSubmissionCount from Tests t left join Attempts a on t.test_id=a.test_id where t.classroom_id=? group by t.title;";
	public static final String getTopPerfomanceStudents="select distinct cu.user_id,name, max(marks) as score from Attempts a join Classroom_Users cu on a.user_id=cu.user_id join Users u on u.user_id=cu.user_id where cu.classroom_id=? and cu.role='student' group by cu.user_id, u.name having score >0 order by score desc";

	public static final String getStudentSubmittedTest="select  t.title, count(a.test_id) as attemptCount, t.created_at,    t.test_id, t.classroom_id ,cu.user_id as userId from Tests t join Classroom_Users cu on t.classroom_id = cu.classroom_id join Attempts a on cu.user_id = a.user_id  and a.test_id = t.test_id where     t.classroom_id = ?  and cu.user_id = ?  group by   t.title,   t.created_at,  t.test_id,   t.classroom_id,cu.user_id having  count(a.test_id) >=1;";
	public static final String deleteStudent="delete from Classroom_Users where user_id = ? and classroom_id = ?";
	...
	public static final String getTestSubmissionDetails = "select u.user_id  , u.name , u.email , count(a.attempt_id) as total_attempts , sum(a.status = 'evaluated') as evaluated , sum(a.status = 'submitted') as submitted  from Users u join Classroom_Users cu on u.user_id = cu.user_id and cu.role != 'tutor' join Tests t on t.classroom_id = cu.classroom_id"
